...
def train():
    ...
    if args.prefetch_forward_layers >= 1:
        model = TransformersModel(model_id=args.model_id, fsdp_config={'transformer_cls_names_to_wrap':['Qwen3NextSparseMoeBlock'], 'forward_prefetch':True})
    else:
        # Use a TransformersModel, transformer_cls_names_to_wrap=Qwen3NextSparseMoeBlock to avoid hang of fsdp2
        model = TransformersModel(model_id=args.model_id, fsdp_config={'transformer_cls_names_to_wrap':['Qwen3NextSparseMoeBlock']})
